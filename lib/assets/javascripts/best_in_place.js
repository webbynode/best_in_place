...
BestInPlaceEditor.forms = {
  "input" : {
    activateForm : function() {
      ...
      var data_value = $(this.element).attr('data-value');

      var to_show_value = this.display_value;
      console.debug('to_show_value', to_show_value);
      console.debug('data_value', data_value);

      if (!(typeof data_value === "undefined")) {
        to_show_value = data_value;
      }
      console.debug('to_show_value', to_show_value);

      var output = '<form class="form_in_place" action="javascript:void(0)" style="display:inline;">';
      output += '<input type="text" name="'+ this.attributeName + '" value="' + this.sanitizeValue(to_show_value) + '"';
